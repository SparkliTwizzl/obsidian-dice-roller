...
test('Lexer should parse "1d6 @\"rollAlias\""', () => {
    let actual = Lexer.parse("1d6 @\"rollAlias\"").unwrap().map(toLexicalToken);
    expect(actual).toEqual([
        {
            conditions: undefined,
            parenedDice: undefined,
...
test('Lexer should not parse "1d6; 1d4 * (1d2 - 1); ~\"|\" @\"rollAlias\"" when ChainRoller is disabled', () => {
    let _prev = Lexer.isChainRollerEnabled;
    Lexer.setEnableChainRoller(false);

    let actual = Lexer.parse("1d6; 1d4 * (1d2 - 1); ~\"|\" @\"rollAlias\"").unwrapErr();
    expect(actual).toEqual("Could not parse");

    Lexer.setEnableChainRoller(_prev);
});

test('Lexer should parse "1d6; 1d4 * (1d2 - 1); ~\"|\" @\"rollAlias\"" when ChainRoller is enabled', () => {
    let _prev = Lexer.isChainRollerEnabled;
    Lexer.setEnableChainRoller(true);

    let actual = Lexer.parse("1d6; 1d4 * (1d2 - 1); ~\"|\" @\"rollAlias\"").unwrap().map(toLexicalToken);
    expect(actual).toEqual([
        {
            conditions: undefined,
            parenedDice: undefined,
...
test('Lexer should not parse "2 (1d6; 1d4)" when ChainRoller is enabled', () => {
    let _prev = Lexer.isChainRollerEnabled;
    Lexer.setEnableChainRoller(true);

    let actual = Lexer.parse("2 (1d6; 1d4)").unwrapErr();
    expect(actual).toEqual("Could not parse");
